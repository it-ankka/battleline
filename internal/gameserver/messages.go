...
func (client *SessionClient) SendSessionMessage(
	...
	message := SessionMessage{MessageType: messageType, Timestamp: time.Now()}
...
func (game *GameSession) HandleClientMoveMessage(m ClientMessage) {
	moveAction := m.Data.Move.Action
	game.mu.Lock()
	defer game.mu.Unlock()

	//TODO
	// if moveAction == PlaceAction && game.GameState.ActivePlayer == m.Client.Index {
	// 	playerHand := game.GameState.PlayerHands[m.Client.Index]
	// 	cardIdx := playerHand.FindCardIdx(*m.Data.Move.Card)
	// 	if cardIdx == -1 {
	// 		m.Client.SendSessionMessage(SessionMessageError, game, &SessionError{Message: "Invalid Move"})
	// 		return
	// 	}
